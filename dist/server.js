...
async function ensureDirectories() {
  if (!fs.existsSync(REPORTS_DIR)) {
    await promises.mkdir(REPORTS_DIR, { recursive: true });
  }
  if (!fs.existsSync(SCREENSHOTS_DIR)) {
    await promises.mkdir(SCREENSHOTS_DIR, { recursive: true });
  }
}
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        if (pending) {
          ...
          const filepath = path.join(REPORTS_DIR, `${pending.filename}.md`);
          await promises.writeFile(filepath, markdown, "utf-8");
  ...
  if (req.method === "POST" && url === "/screenshot") {
    try {
      ...
      if (pending) {
        ...
        await promises.writeFile(screenshotPath, buffer);
        console.log(`[ContextReporter] Saved screenshot: ${screenshotPath}`);
        const markdown = generateMarkdown(pending.report, `${filename}.png`);
        const reportPath = path.join(REPORTS_DIR, `${filename}.md`);
        await promises.writeFile(reportPath, markdown, "utf-8");
